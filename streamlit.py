...
temp = st.slider("예상 기온(섭씨)", min_value = -30.0, max_value=45.0, value=25.0)
...
if st.button("수요 예측"):
    try:
        model = joblib.load("demand_elec_model.pkl")
        input_data = np.array([[temp, sun, rain, wind, humid]])
        prediction = model.predict(input_data)[0]
        
        st.write(f"당일 전력 수요량 예측 결과: {prediction:.2f} MW")
        
    except FileNotFoundError :
        st.error("오류: 'demand_elec_model.pkl' 파일을 찾을 수 없습니다. 모델 파일이 앱과 같은 디렉터리에 있는지 확인해주세요.")
